...
awk '/^VALIDATOR_EVIDENCE pcpq1_.* class=active / {
    split($4, finalization, "=");
    split($5, commit_phase, "=");
    if (finalization[2] == 0) exit 1;
    if (commit_phase[2] != 0) exit 1;
}' "$base/launch-report.out"
