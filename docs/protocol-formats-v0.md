...
- Build a byte payload:

```text
String("primechain-dev-tx-root-v0")
...
fixed-2-of-3-mldsa65-rounds-locks-v4
```
...
`fixed-2-of-3-mldsa65-v2`. Later rounds generated by the current implementation
use `fixed-2-of-3-mldsa65-rounds-locks-v4`, which carries cross-round lock
evidence. The older `fixed-2-of-3-mldsa65-rounds-v3` rule is retained for
historical replay compatibility but is no longer generated for new later-round
finalizations.
...
- current later-round finalizations use `fixed-2-of-3-mldsa65-rounds-locks-v4`,
- historical v3 later-round records are accepted for replay compatibility,
- validators persist one signed candidate per `(frontier integer, round)` to prevent equivocation across restart.

For v4, the candidate hash signed by validators is the stable underlying
candidate-subject hash with finalization evidence excluded. This lets the same
underlying prime or composite candidate be carried into a later recovery round
with different round-change evidence while still being recognized as the same
candidate. Older v2/v3 records use their historical vote-target rules.
...
cross-round candidate switching. Current later-round finalization uses the v4
locked round-change rule:
...
RoundChangeVote {
    ...
    new_round: UInt64,
    locked_round: UInt64,
    locked_candidate_kind: String,
    locked_candidate_hash: Hash256,
    locked_candidate_payload: Bytes,
    signature: Bytes
}
```

If `locked_round == 0`, the validator carries no prior lock and the locked
candidate fields must be empty/zero. If `locked_round > 0`, the lock must refer
to a prior round lower than `new_round`, `locked_candidate_kind` must be `PRIME`
or `COMPOSITE`, `locked_candidate_payload` must deserialize as that candidate
kind, and `locked_candidate_hash` must equal the stable subject hash computed
from that payload.

The v4 signed payload is:

```text
Encode(
    "primechain-finalization-round-change-locks-mldsa65-v4",
    previous_record_hash,
    integer,
    new_round,
    locked_round,
    locked_candidate_kind,
    locked_candidate_hash,
    locked_candidate_payload,
    validator_address
)
```

Two or three distinct active validators must sign the same frontier hash,
integer, and target round. The proposer then applies the highest-lock rule:

- find the highest `locked_round` among the round-change votes,
- if no vote carries a lock, a fresh valid candidate may be finalized,
- if exactly one candidate hash appears at the highest locked round, the later
  round must finalize that same candidate,
- if different candidate hashes appear at the same highest locked round, the
  certificate is conflicting evidence and the later-round finalization is
  rejected.

The full locked candidate payload is carried in the round-change vote so a new
proposer can recover and repropose the safe candidate even if it never saw the
original round's candidate message. The finalized v4 record embeds the canonical
round-change votes and is permanent record history.

The older v3 round-change format carried only the frontier hash, integer, target
round, validator address, and signature. It remains accepted only for historical
records already produced before v4 locking. New later-round records should use
`fixed-2-of-3-mldsa65-rounds-locks-v4`.

Temporary votes are atomically stored in `<record-store>.rounds`; per-round
signed choices and locked candidate payloads remain in `<record-store>.finalization`.
Both sidecars are recovery aids for live validators and are not needed for
historical replay once the finalized record is in `chain.dat`.
