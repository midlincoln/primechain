...
Status: living document, fourth pass, first-pass scope now substantially complete (pass 1: types/tx/proof formats and record fields, structurally; pass 2: §8/§8.1 finalization and round-change, traced in detail; pass 3: §10/§10.1-10.3 commit-phase quorum, genesis anchor, epoch transitions, traced in detail; pass 4/this pass: §3 transaction validation and §9.1 authenticated address derivation, both traced in detail). Analysis and documentation only -- **nothing in the source tree was changed while writing this**, and no finding below has been acted on. Scope: `docs/protocol-formats-v0.md` (both repos are `kaito-zero/primechain`'s `docs/protocol-gap-analysis` branch, based on `upstream/main` @ `1134489`) checked against the actual C++ implementation.
...
| §3 | Transaction format + validation rules (positive integer amounts, referenced primes must be finalized, sufficient balance, sum-in==sum-out+fee, nonce starts at 1 and increments by exactly one, post-batch fee crediting, overflow rejection, no floats) | **Traced in detail this pass (fourth pass), superseding the note below -- this section matches the document almost exactly, line for line.** `microUnits()` (`sequential_node.cpp:89`) requires `denominator == 1 && numerator > 0`, exactly the doc's rule, with the same fallthrough for input and output amounts. The fee rule (`denominator == 1`, zero valid, non-zero must name `prime >= 2`) is a single `if` whose condition reads almost like a transcription of the doc's sentence. `checkedAdd()` guards every accumulation (input sum, output sum, fee, balances, total supply) against uint64 overflow, matching "amount aggregation must reject unsigned 64-bit overflow" exhaustively rather than at just one point. `debits != credits` (built by prime asset across inputs vs. outputs+fee) is the sum-in-equals-sum-out-plus-fee check, and its error text ("transaction inputs must equal outputs plus fee for each prime asset") nearly quotes the document. Nonce: `expected_nonce = accountNonce(sender) + 1`, rejects unless `tx.nonce == expected_nonce` -- matches "starts at 1, exactly previous plus one" (a fresh sender's `accountNonce` is 0, so the first accepted nonce is 1). Batch fee crediting happens in a second pass strictly after every transaction in the batch is applied (`collected_fees` accumulated during the loop, only credited to the fee recipient afterward), matching "a transaction in the same batch cannot spend those newly collected fees" precisely. Authenticated signing: `verifyAuthenticatedTransactionSignature` checks address-derives-from-public-key then verifies against `transactionSigningPayload()`, whose domain string is `"primechain-transaction-signature-mldsa65-v2"` -- an exact byte-for-byte match against the document's stated domain. One rule is enforced **implicitly rather than explicitly**: "all referenced prime values must already have finalized prime records" has no dedicated check found this pass -- it holds structurally, because a prime's balance ledger only ever gets entries via `applyPrimeLedger()` crediting a finalized prime record, so `debit()`'s "insufficient balance" check will always reject a reference to a never-finalized prime as a side effect, without a rule that says so directly. Functionally equivalent, but worth knowing if anyone goes looking for an explicit check and doesn't find one. | `sequential_node.cpp:89-1386` (`microUnits`, `checkedAdd`, `credit`/`debit`, the main per-transaction validation loop), `records.cpp`/`crypto/signature.cpp` (signing/verification) | Detailed -- essentially fully verified against the document |
...
| §9.1 | ML-DSA-65 composite contributor auth (address = pcpq1 + first 20 bytes of hash(pubkey), domain-separated commit/reveal payloads) | **Resolved this pass -- the "genuinely unclear" flag below is cleared.** The authenticated address derivation is `addressFromProtocolPublicKey()` → `addressFromPublicKey()` (`crypto/signature.cpp:142`): `"pcpq1_" + toHex(sha3_256(public_key)).substr(0, 40)` -- 40 hex characters is exactly the first 20 bytes, an exact match to §2's stated format. This same function is what §8's `verifyRecordFinalization` and §10.3's `verifyValidatorEpochTransition` both use for their own address-derives-from-key checks, so it's a single shared, consistently-used implementation rather than several independent ones that could drift apart. Small bonus finding while tracing this: the same file also defines a second, parallel address scheme -- `SignatureAlgorithm::Ed25519` with prefix `"pc1_"` (vs. `"pcpq1_"` for the documented ML-DSA-65 scheme) -- with zero mentions anywhere in `protocol-formats-v0.md`. A targeted search found no caller anywhere in `tools/` or `wallet/` that actually constructs an Ed25519 keypair or a `pc1_` address, so this looks like unused/vestigial code (plausibly predating the switch to post-quantum ML-DSA-65) rather than a live undocumented mechanism -- much lower priority than Headline Finding 2's five mechanisms, not elevated to its own headline finding, but worth knowing it exists if anyone's doing a security-relevant audit of the address-format space. Commit/reveal domain strings (§9.1's `CommitSignaturePayload`/`RevealSignaturePayload`) were not independently re-diffed against `sync_server.cpp`'s actual construction this pass. | `crypto/signature.cpp:142-180` | Detailed for address derivation; commit/reveal payload domain strings still not independently verified |
...
## What this pass did NOT cover (explicitly, so any future pass knows where to start)

Every numbered section of `protocol-formats-v0.md` (§2 through §10.3) has now been traced at least structurally, and the consensus-critical ones (§3, §8, §9.1, §10) in real line-by-line detail. What's left is narrower and more mechanical than what's been covered:

- No verification of exact byte-level field ordering for every serialized structure against the document's field lists (spot-checked for a few structures via the functions that touch them; not exhaustively diffed field-by-field for all of them).
- Composite proof (§5) and Pratt proof (§6) validation rules were confirmed structurally (structs match, key helper functions like `gcd()` exist and are called) but not traced with the same line-by-line rigor given to §3/§8/§9.1/§10 this pass.
- Commit/reveal payload domain strings (§9.1, §10) not independently re-diffed against construction sites in `sync_server.cpp`.
...
- The `SUBMIT_PHASE_VOTE_BUNDLE`/`*_PEER` wire commands (found while tracing §10.1) and the apparently-unused `pc1_`/Ed25519 address scheme (found while tracing §9.1) were noticed but not investigated further.

This remaining work is exactly what Phase D (deterministic conformance vectors) and Phase E (C++ conformance tests) are for -- byte-level, field-by-field verification is naturally done *by writing the vectors themselves*, not as a separate reading pass. Continuing to hand-trace more sections here has diminishing returns now that the consensus-critical core (transaction validation, finalization, commit-phase quorum, genesis, epoch transitions, address derivation) is solidly verified.

## Recommended next task

Four headline findings are fully diagnosed with specific file/line evidence and need a maintainer decision, not more tracing. Given Finding 4's severity (the actual safety margin of the live 3-validator network), **the recommended next step is still (b): take Findings 1, 3, and 4 to the maintainer** -- this hasn't changed from the previous pass, but it's now on firmer ground, since the sections traced in this pass (§3, §9.1) turned out to match the document closely, which is itself useful context: the four findings aren't symptomatic of the spec being broadly unreliable, they're four specific, isolated things.

With this pass, the gap-analysis document has covered its originally-scoped ground. The next distinct piece of work, per this session's separate roadmap discussion, is **Phase H (fuzzing)** -- and this pass directly informs where to start it: §3's now-precisely-understood transaction validation (`microUnits()`'s positive-integer-micro-units check, the overflow-guarded accumulation, the exact nonce-sequencing rule) is a well-specified, self-contained target for a first fuzz harness, more so than guessing at it would have been before this pass.
